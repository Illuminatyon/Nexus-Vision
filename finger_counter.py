...
class FingerCounter:
    ...
    def draw_finger_status(self, frame, hand_landmarks, extended_fingers, is_left):
        ...
        cv2.putText(frame, f"{hand_label}: {finger_count} fingers", 
                   ...
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, landmark_color, 1)
        ...
        if 0 in extended_fingers:  # If thumb is in the list (meaning it's NOT physically extended with inverted behavior)
            ...
            # Add thumb debug text
            thumb_tip_x = int(thumb_tip.x * w)
            thumb_tip_y = int(thumb_tip.y * h)
            debug_color = color_left if is_left else color_right
            cv2.putText(frame, "Thumb tucked in (counted)", 
                       (thumb_tip_x - 60, thumb_tip_y - 15), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, debug_color, 1)
        ...
        for finger_idx in extended_fingers:
            ...
            # Use different colors for thumbs to highlight them
            circle_color = (0, 255, 255)  # Yellow for most fingers
            if finger_idx == 0:  # Thumb on either hand
                if is_left:
                    circle_color = (0, 255, 0)  # Green for left hand thumb
                else:
                    circle_color = (255, 0, 255)  # Magenta for right hand thumb
